...
pub(super) async fn execute_debug_command(
    ...
    if trimmed == "reload" {
        let repo_dir = crate::build::get_repo_dir()
            .ok_or_else(|| anyhow::anyhow!("Could not find repository directory"))?;

        let target_binary = crate::build::find_dev_binary(&repo_dir)
            .unwrap_or_else(|| build::release_binary_path(&repo_dir));
        if !target_binary.exists() {
            let self_dev_cmd = crate::product::command_with("self-dev --build");
            return Err(anyhow::anyhow!(format!(
                "No binary found at {}. Run '{}' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p jcode --bin {}' and publish current.",
