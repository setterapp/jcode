...
fn source_build_repo_dir() -> Result<PathBuf> {
    Ok(source_build_root()?.join(crate::product::binary_stem()))
}
...
/// Build the active product binary from source by cloning/pulling the repo and running cargo build.
fn build_from_source() -> Result<PathBuf> {
    ...
    if repo_dir.join(".git").exists() {
        ...
        let repo_checkout_dir = crate::product::binary_stem();
        let output = std::process::Command::new("git")
            .args([
                "clone",
                "--depth",
...
pub fn download_and_install_blocking_with_progress(
    ...
    if asset.name.ends_with(".tar.gz") {
        ...
        for entry in archive.entries()? {
            ...
            entry.unpack(&dest)?;
            if file_name.starts_with(crate::product::binary_stem()) && !file_name.ends_with(".bin")
            {
